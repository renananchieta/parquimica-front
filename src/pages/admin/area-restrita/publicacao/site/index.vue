        ...
        :items="items">
            <template v-slot:top>
                <v-toolbar flat>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn variant="elevated" color="primary" to="/admin/area-restrita/publicacao/site/nova-publicacao">
...
const getPostagensSite = () => {
    ...
    api.get('/area-restrita/site/postagem/grid', {params: form.value})
    .then((response) => {
...
const editItem = (item) => {
    router.push(`/admin/area-restrita/publicacao/site/${item.id}`)    
}
...
const confirmarRemoverPublicacao = (item) => {
    ...
    api.delete(`/area-restrita/site/postagem/${item.id}`)
    .then((response) => {
